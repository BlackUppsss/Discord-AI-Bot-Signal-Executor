...
def _supports_watch_ticker(exchange):
    return bool(getattr(exchange, "has", {}).get("watchTicker"))


def _ticker_price(ticker):
    price = ticker.get("last") or ticker.get("close")
    if price is None:
        raise ValueError(f"Ticker tidak berisi last/close price: {ticker}")
    return float(price)
...
async def price_monitor_task(symbol_ccxt, side, entry_price, sl_price, order_type="MARKET"):
    ...
    if not _supports_watch_ticker(exchange):
        print(f"[AUTO BE WS] Error: {exchange.id} watchTicker() tidak tersedia di ccxt.pro.")
        return
...
def cancel_monitor(symbol_ccxt):
    """Membatalkan task price monitor yang sedang berjalan untuk koin tertentu."""
    task = _active_monitors.get(symbol_ccxt)
    if task and not task.done():
        print(f"[AUTO BE] Membatalkan monitor harga untuk {symbol_ccxt} karena sinyal CLOSE.")
        task.cancel()
